...
* 本專案為引導使用者將路段編碼導入本身系統中，共開發單路段選擇工具(設定1組坐標)、多路段選取工具(設定2組坐標)、多路段選取工具(不設定起始資訊)、方位角工具(設定1組座標)，共以上四項。

* 工具適用時機：

  * 單路段選擇工具(設定1組座標)：透過單一設備(如VD、CCTV、CMS)坐標取得LinkID相關資訊。
 
  * 多路段選取工具(設定2組座標)：透過起迄坐標建立路徑LinkID (如eTag配對路徑)相關資訊。
 
  * 多路段選取工具(不設定起使資訊)：供路徑匹配使用，可直接於圖面點選連續路段以建立路徑相關資訊。
  
  * 方位角工具(設定1組座標)：透過單一設備(如站牌、標誌)坐標取得所在路段之Bearing資訊。
...
* 輸入/輸出欄位：
  ...
  * 單路段選擇工具-輸出欄位：LinkID、Bearing、 RoadID、RoadName、RoadClass、RoadDirectionID、Version。
  
  * 多路段選取工具-輸出欄位：LinkID、Bearing、 RoadID、RoadName、RoadClass、RoadDirectionID、Version。

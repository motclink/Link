...
* 本專案為引導使用者將路段編碼導入本身系統中，共開發方位角工具、單路段選擇工具(設定1組坐標)、多路段選取工具(設定2組坐標)、多路段選取工具(不設定起始資訊)，共以上四項。

* 工具適用時機：

  * 方位角工具：供僅需導入設備方位角位置之設備使用，如公車站牌…等。
 
  * 單路段選擇工具(設定1組坐標)：供僅需導入路段方位角、LinkID、RoadID…等資料設備使用，如VD、CCTV…等。
 
  * 多路段選取工具(設定2組坐標)：供需導入路徑(連續路段)之方位角、LinkID、RoadID…等資料設備使用，如ETC…等。
  
  * 多路段選取工具(不設定起始資訊)：供需導入路徑(連續路段)之方位角、LinkID、RoadID…等資料設備使用，如ETC…等。
...
* 輸入/輸出欄位：
  ...
  * 單路段選擇工具(設定1組坐標)-輸出欄位：LinkID、Bearing、 RoadID、RoadName、RoadClass、RoadDirection、Version。
  
  * 多路段選取工具(設定2組坐標)-輸出欄位：LinkID、Bearing、 RoadID、RoadName、RoadClass、RoadDirection、Version。
  
  * 多路段選取工具(不設定起始資訊)-輸出欄位：LinkID、Bearing、 RoadID、RoadName、RoadClass、RoadDirection、Version。
...
* 完成匯入後可進行工具選擇，在本頁選擇多路段選取工具（設定2組坐標）後點擊下一步。
...
* 如未選擇選完畢則點選「完成編輯」，直到迄點與起點相接。

* 如已選擇完畢則點選「繼續選取」，即可編輯下一筆資料。
...
* 完成匯入後可進行工具選擇，在在本頁選擇多路段選取工具（不設定起始資訊）後點擊下一步。
...
* 如方位未選擇選完畢則點選「繼續選取」，即可繼續編輯該筆資料。

* 如以選擇完畢則點選「完成編輯」，則會跳至下一筆。
